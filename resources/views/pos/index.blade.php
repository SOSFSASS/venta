    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="icon-head text-center">
                    <a href="javascript:void(0);">
                        <img src="{{ asset('css/img/logo3.png') }}" width="100" height="30" alt="Receipt Logo">
                    </a>
                </div>
                <div class="text-center info">
                    <h6>Dreamguys Technologies Pvt Ltd.,</h6>
                    <p class="mb-0">Phone Number: +1 5656665656</p>
                    <p class="mb-0">Email: <a href="mailto:support@example.com">support@example.com</a></p>
                </div>
                <div class="tax-invoice">
                    <h6 class="text-center">Tax Invoice</h6>
                    <div class="row">
...
@push('js')
    ...
    <script>
        ...
        //         $('#print-receipt .invoice-user-name').eq(0).find('span').eq(1).text(pedido.cliente);
        //         $('#print-receipt .invoice-user-name').eq(1).find('span').eq(1).text(pedido.documento);
        //         $('#print-receipt .invoice-user-name').eq(2).find('span').eq(1).text(pedido.correo);

        //         var productosHtml = '';
        //         pedido.productos.forEach(function(producto, index) {
        //             productosHtml += `
        //     <tr>
        //         <td>${index + 1}. ${producto.id}</td>
        //         <td>S/.${producto.costo_venta.toFixed(2)}</td>
        ...
        //         $('#print-receipt tbody').html(productosHtml);

        //         var igv = pedido.igv.toFixed(2);
        //         var total = pedido.total.toFixed(2);

        //         $('#print-receipt .table-borderless').eq(1).find('tr').eq(0).find('td').eq(1).text(
        //             `S/. ${pedido.subtotal.toFixed(2)}`);
        //         $('#print-receipt .table-borderless').eq(1).find('tr').eq(1).find('td').eq(1).text(`S/. ${igv}`);
        //         $('#print-receipt .table-borderless').eq(1).find('tr').eq(2).find('td').eq(1).text(`S/. ${total}`);
        //     }
                // Función para actualizar el modal de impresión del recibo
                function actualizarModalRecibo(pedido) {
